...
<details>
<summary>작업 스케줄러 관리</summary>

| 항목 | 명령어 |
|---|---|
| 작업 목록 조회 | `Get-ScheduledTask \| Where-Object { $_.TaskPath -eq '\' } \| Sort-Object TaskName` |
| 작업 삭제 | `Unregister-ScheduledTask -TaskName '작업명' -TaskPath '\' -Confirm:$false` |

- 루트(`\`) 경로에 등록된 작업만 조회 (광고성 소프트웨어나 3rd party가 주로 루트에 등록)
- `\Microsoft\` 하위 Windows 시스템 작업은 조회 대상에서 제외됨
- 관리자 권한으로 실행 시 일반 권한에서 보이지 않는 작업까지 표시
- 작업 목록을 백그라운드(`SwingWorker`)에서 비동기로 로드
- 체크박스로 항목 선택 후 삭제, 삭제는 관리자 권한 필요
- 삭제 완료 후 목록 자동 갱신
...
<details>
<summary>하드디스크 상태 점검</summary>

| 항목 | 명령어 |
|---|---|
| 디스크 목록 조회 | `Get-PhysicalDisk \| ForEach-Object { $_.DeviceId + '\|' + $_.FriendlyName + '\|' + [math]::Round($_.Size / 1GB) }` |
| 온도 조회 | `($d \| Get-StorageReliabilityCounter).Temperature` |
| 사용시간 / 전원 인가 횟수 | `$rc.PowerOnHours` / `$rc.StartStopCycleCount` (`$rc = $d \| Get-StorageReliabilityCounter`) |

- 물리 디스크 목록을 콤보박스로 표시하고, 선택 시 해당 디스크 정보를 백그라운드(`SwingWorker`)에서 조회
- 온도는 0~50°C 정상, 51~60°C 약간 높음, 60°C 이상 위험으로 색상 구분하여 표시
- 사용시간·전원 인가 횟수는 SMART(`Get-StorageReliabilityCounter`) 기반으로, 관리자 권한이 필요
- NVMe 등 일부 드라이브는 드라이버 한계로 사용시간·전원 인가 횟수 값이 제공되지 않을 수 있으며, 이 경우 "측정 불가"로 표시
